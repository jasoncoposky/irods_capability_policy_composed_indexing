...
def purge_event_handler_configured(arg=None):
                ...
                'plugin_specific_configuration': {
                    "policies_to_invoke" : [
                        ...
                        },
                        {
                            ...
                            "events" : ["unregister", "unlink", "rename"],
                            "conditional" : {
                                "metadata_exists" : {
                                    "recursive" : "true",
                                    "attribute" : "irods::indexing::index",
...
class TestElasticSearchPurgeFullText(ResourceBase, unittest.TestCase):
    ...
    def setUp(self):
        super(TestElasticSearchPurgeFullText, self).setUp()

    def tearDown(self):
        super(TestElasticSearchPurgeFullText, self).tearDown()

    def test_purge_rm_file(self):
        self.repave_index()
        with session.make_session_for_existing_admin() as admin_session:
            physical_path = '/var/lib/irods/scripts/irods/test/full_text_purge_test_file.txt'
            logical_path  = '/tempZone/home/rods/full_text_purge_test_file.txt'
            admin_session.assert_icommand('imeta set -C /tempZone/home irods::indexing::index full_text_index::full_text elasticsearch')

            try:
                with purge_event_handler_configured():
